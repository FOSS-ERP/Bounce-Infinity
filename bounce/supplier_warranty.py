...
def populate_warranty_claim(doc: Document, method: str | None = None):
	...
	for field in ("custom_supplier", "custom_original_purchase_receipt", "custom_original_purchase_rate", "custom_purchase_return", "custom_supplier_debit_note"):
		doc.set(field, serial.get(field))
	doc.custom_supplier_claim_multiplier = serial.custom_supplier_claim_multiplier or 2
	doc.custom_supplier_claim_amount = flt(doc.custom_original_purchase_rate) * flt(doc.custom_supplier_claim_multiplier)
...
def _find_purchase_receipt_item(pr: Document, serial_no: str, item_code: str):
	for row in pr.items:
		if row.item_code != item_code:
			continue
		if row.serial_and_batch_bundle and frappe.db.exists("Serial and Batch Entry", {"parent": row.serial_and_batch_bundle, "serial_no": serial_no}):
			return row
...
def _calculate_claim(doc: Document):
	if not doc.is_new() and doc.has_value_changed("custom_supplier_claim_multiplier") and not {
		"Warranty Manager",
		"System Manager",
	}.intersection(frappe.get_roles()):
...
def _get_serial(serial_no: str):
	...
	if not doc.custom_supplier or not doc.custom_original_purchase_receipt or not doc.custom_original_purchase_receipt_item:
...
def create_supplier_return(serial_no: str, warranty_claim: str | None = None):
	...
	if serial.custom_purchase_return and frappe.db.get_value("Purchase Receipt", serial.custom_purchase_return, "docstatus") != 2:
		frappe.throw(_("Supplier Warranty Return already exists: {0}").format(serial.custom_purchase_return))
	if not serial.warehouse:
		frappe.throw(_("Serial No {0} must be received into a warehouse before supplier return.").format(serial.name))
...
def create_warranty_debit_note(serial_no: str, warranty_claim: str | None = None):
	...
	if serial.custom_supplier_debit_note and frappe.db.get_value("Purchase Invoice", serial.custom_supplier_debit_note, "docstatus") != 2:
	...
	invoice = frappe.get_doc({
		...
		"posting_date": today(), "is_return": 1, "update_stock": 0,
		"custom_supplier_warranty_serial": serial.name, "custom_warranty_claim": warranty_claim,
		"items": [{"item_code": serial.item_code, "qty": -1, "rate": serial.custom_supplier_claim_amount,
			"expense_account": pr_item.expense_account, "cost_center": pr_item.cost_center}],
	})
	invoice.insert()
...
def sync_purchase_return(doc: Document, method: str | None = None):
	if doc.is_return and doc.custom_supplier_warranty_serial:
		...
		frappe.db.set_value("Serial No", doc.custom_supplier_warranty_serial, "custom_supplier_claim_status", status)
